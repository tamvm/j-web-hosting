...
class AcesefModelAcesef extends AcesefModel {
	...
	function sefStatus() {
        ...
        $types = array('version_checker', 'sef', 'mod_rewrite', 'live_site', 'jfrouter', 'acesef', 'plugin', 'generate_sef');
        $msg = '';
        ...
        if (in_array($type, $types)) {
            ...
            if ($type == 'sef' || $type == 'mod_rewrite' || $type == 'live_site') {
                ...
                if ($type == 'sef') {
                    $JoomlaConfig->setValue('sef', $value);
                } 
				elseif ($type == 'mod_rewrite') {
                    $JoomlaConfig->setValue('sef_rewrite', $value);
                } 
				elseif ($type == 'live_site') {
					...
					$JoomlaConfig->setValue('live_site', trim($live_site));
                }
            ...
            elseif ($type == 'plugin' || $type == 'jfrouter') {               
                if ($type == 'plugin') {
                    $type = 'acesef';
                }
                
                if (!AceDatabase::query("UPDATE `#__plugins` SET `published` = '{$value}' WHERE (`element` = '{$type}') AND (`folder` = 'system') LIMIT 1")) {
	...
	function getInfo() {
		...
		if (!isset($info)) {
			...
			if ($this->AcesefConfig->version_checker == 1){
				$info['version_installed'] = AcesefUtility::getXmlText(JPATH_ACESEF_ADMIN.DS.'acesef.xml', 'version');
