...
class AcesefModelConfig extends AcesefModel {
	...
	function save() {
		...
		$config->seo_h1							= JRequest::getVar('seo_h1', 						0, 			'post', 'int');
        ...
        if ($id = AceDatabase::loadResult('SELECT id FROM #__content WHERE `title` = "404"')){
			...
			$sql = 'INSERT INTO #__content (title, alias, introtext, `fulltext`, state, sectionid, mask, catid, created, created_by, created_by_alias, modified, modified_by, checked_out, checked_out_time, publish_up, publish_down, images, urls, attribs, version, parentid, ordering, metakey, metadesc, access, hits) '.
			'VALUES("404", "404", "'.$introtext.'", "", "1", "0", "0", "0", "2004-11-11 12:44:38", "62", "", "'.date("Y-m-d H:i:s").'", "0", "62", "2004-11-11 12:45:09", "2004-10-17 00:00:00", "0000-00-00 00:00:00", "", "", "menu_image=-1\nitem_title=0\npageclass_sfx=\nback_button=\nrating=0\nauthor=0\ncreatedate=0\nmodifydate=0\npdf=0\nprint=0\nemail=0", "1", "0", "0", "", "", "0", "750");';
        }
	...
	function getLists() {
		...
		*/
		...
		$lists['seo_h1'] 			= JHTML::_('select.booleanlist', 'seo_h1', null, $this->AcesefConfig->seo_h1);
