...
class AcesefModelExtensions extends AcesefModel {
	...
	function checkComponents() {
		...
		foreach ($components as $component) {
			...
			$total = AceDatabase::loadResult("SELECT COUNT(*) FROM #__acesef_extensions WHERE extension = '".$component->option."'");
			...
			if ($total < 1) {
				...
				if(!$routed){
					$ext = JPATH_ACESEF_ADMIN.DS.'extensions'.DS.$component->option.'.php';
					if (file_exists($ext)) {
						$name = AcesefUtility::getXmlText(JPATH_ACESEF_ADMIN.DS.'extensions'.DS.$component->option.'.xml', 'name');
						$router = 3;
						$routed = true;
					}
				}
				...
				if($routed){
					...
					$params .= "\nprefix=";
					$params .= "\nskip_menu=0";
					AceDatabase::query("INSERT INTO #__acesef_extensions (name, extension, params) VALUES ('{$name}', '{$component->option}', '{$params}')");
				}
			}
		}
	}
