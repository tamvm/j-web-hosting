...
class AcesefUtility {
	...
	function getRemoteInfo() {
		...
		if (!is_array($information)) {
			...
			if (strstr($components, '<?xml version="1.0" encoding="UTF-8" ?>')) {
				...
				if (!is_object($manifest) || ($manifest->name() != 'versions')) {
					unset ($xml);
					return $information;
				}
				
				$cat = $manifest->getElementByPath('category');
				if (!is_a($cat, 'JSimpleXMLElement') || (count($cat->children()) == 0)) {
					return $information;
				}
				
				foreach ($cat->children() as $extension) {
					$option = $extension->attributes('option');
					$compability = $extension->attributes('compability');

					if ($option == 'com_acesef' && ($compability == '1.5' || $compability == '1.5_1.6')) {
	...
	function storeConfig($AcesefConfig) {
		...
		$db =& JFactory::getDBO();
		$db->setQuery('UPDATE #__components SET params = "'.$config.'" WHERE link = "option=com_acesef" AND parent = 0');
		$db->query();
	}
    ...
    function getSefStatus() {
        ...
        if (!isset($status)) {
            ...
            $status['live_site'] = !is_null($JoomlaConfig->getValue('live_site')) ? $JoomlaConfig->getValue('live_site') : '';
