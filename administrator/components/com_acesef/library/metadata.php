...
class AcesefMetadata {
		...
		if (!empty($urls)) {
			...
			foreach ($urls as $i => $record) {
				...
				if (file_exists(JPATH_ACESEF_ADMIN.DS.'extensions'.DS.$component.'.php')) {
					...
					$acesef_ext->build($uri->_vars, $segments, $do_sef, $meta, $item_limitstart);
	...
	function plugin($document) {
		...
		if ($this->AcesefConfig->meta_core == 1) {
			...
			// Title
			$title = self::_pluginTitle($org_title, $meta_title);
			if (!empty($title)) {
			...
			// Description
			$description = self::_pluginDescKey($org_desc, $meta_desc, $auto_desc, 'desc');
			...
			// Keywords
			$keywords = self::_pluginDescKey($org_key, $meta_key, $auto_key, 'key');
			if (!empty($keywords)) {
	...
	// Title
	function _pluginTitle($org_title, $db_title) {
		if (!empty($db_title)) {
			return $db_title;
		}
	...
	function _pluginDescKey($org_val, $db_val, $auto_val, $type) {
		if ($type == 'desc') {
			$AConfig = 'meta_desc';
			$JConfig = 'MetaDesc';
		} elseif ($type == 'key') {
