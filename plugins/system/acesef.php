...
class plgSystemAcesef extends JPlugin {
	...
	function onPrepareContent(&$article, &$params = null, $limitstart = 0) {
		...
		AcesefPlugin::onPrepareContent($article->text);
		
		return;
	}
	
	function onBeforeDisplayContent(&$article, &$params = null, $limitstart = 0) {
		if (!self::_systemCheckup()) {
			return;
		}
		...
		AcesefPlugin::onBeforeDisplayContent($article->title);
